    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlySupporter(address _borrower) {
        require(
            msg.sender == options[_borrower].supporter,
            "RCO: Only supporter"
        );
        _;
    }
    ...
    function initializeOption(
        ...
        require(
            _maturityDuration >= MIN_MATURITY &&
                _maturityDuration <= MAX_MATURITY,
        ...
        // Use CCR + 3% for option initialization threshold
        uint256 strikeThreshold = CCR + (3e16); // CCR + 3% (0.03 * 1e18)
        require(
            troveManager.getCurrentICR(_borrower, price) <= strikeThreshold,
            "RCO: Trove is healthy"
        );
    ...
    function exercise(
        address _borrower
    ) external nonReentrant optionExists(_borrower) onlySupporter(_borrower) {
        BackstopOption storage option = options[_borrower];
        ...
        require(
            collateralValue >= strikePrice,
            "RCO: Exercise not profitable, A(T) < K"
        );
        ...
        require(
            musdToken.balanceOf(msg.sender) >= debtToPayBySupporter,
            "RCO: Insufficient mUSD for strike"
        );
    ...
    function _calculateLambda(
        ...
        uint256 recoveryValue = (liquidationValue * recoveryFraction) /
            DECIMAL_PRECISION;
